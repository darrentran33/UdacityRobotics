    ...
    simple_arm::GoToPosition::Response& res)
{
    ...
    // Wait 3 seconds for arm to settle, so arm has enough time to move to requested position
...
{
    // Initialize the arm_mover node and create a handle to it, instantiated to communicate with ROS
    ros::init(argc, argv, "arm_mover");
    ros::NodeHandle n;

    // Define two publishers to publish std_msgs::Float64 messages on joints respective topics,d efined globally to be accessible from other functions
    ...
    // Define a safe_move service with a handle_safe_move_request callback function, publishes clamp boundaries
